...
exports.listCommentsForNews = listCommentsForNews;
exports.insertComment = insertComment;
exports.commentExists = commentExists;
let newsCommentsColumnsCache = null;
const NEWS_COMMENTS_COL_CACHE_MS = 60000;
async function getNewsCommentsColumns(pool) {
    const now = Date.now();
    if (newsCommentsColumnsCache &&
        now - newsCommentsColumnsCache.at < NEWS_COMMENTS_COL_CACHE_MS) {
        return newsCommentsColumnsCache.cols;
    }
    const [rows] = await pool.query("SHOW COLUMNS FROM news_comments");
    const cols = new Set(rows.map((r) => String(r.Field)));
    newsCommentsColumnsCache = { at: now, cols };
    return cols;
}
async function listCommentsForNews(pool, params) {
...
async function insertComment(pool, params) {
    const cols = await getNewsCommentsColumns(pool);
    const insertCols = [];
    const placeholders = [];
    const values = [];
...
async function commentExists(pool, commentId) {
    const [rows] = await pool.query("SELECT id FROM news_comments WHERE id = ? LIMIT 1", [commentId]);
