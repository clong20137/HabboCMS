...
const requireRank_1 = require("./requireRank");
const router = (0, express_1.Router)();
// rank 4+ to access housekeeping wordfilter
router.use(auth_1.requireAuth, (0, requireRank_1.requireRank)(4));
...
`, params);
        ...
        const [countRows] = await db_1.pool.query(`SELECT COUNT(*) AS cnt FROM wordfilter ${search ? "WHERE word LIKE ? OR replacement LIKE ? OR addedby LIKE ?" : ""}`, search ? [`%${search}%`, `%${search}%`, `%${search}%`] : []);
