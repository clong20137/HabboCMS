...
function withBase(url: string) {
  ...
  const normalizedBase = APP_BASE.endsWith("/")
    ? APP_BASE.slice(0, -1)
    : APP_BASE;
...
export default function Me() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/hk/settings", {
          credentials: "include",
        });
        ...
        const found = (data.items || []).find(
          (x) => x.key === "discord_invite",
        );
        setDiscordInvite(String(found?.value ?? ""));
      } catch {
        if (!alive) return;
        setDiscordInvite("");
      }
  ...
  const avatarUrl = getAvatarUrl(user.figure, { size: "l" });
          ...
          <section className="panel panel-side">
            ...
            <div className="panel-body">
              <div className="discord-row">
                <div className="muted">
                  Join our Discord server to stay updated with the community.
                </div>
                <button
                  ...
                  disabled={!discordInvite}
                >
                  {discordInvite ? "Join Discord" : "Discord Not Configured"}
                </button>
              </div>
            </div>
          </section>
